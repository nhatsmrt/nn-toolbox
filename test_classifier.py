...
model = Sequential(
    ConvolutionalLayer(in_channels=3, out_channels=16, kernel_size=7, activation=nn.ReLU),
    ...
    SEResNeXtShakeShakeAttention(num_heads=8, in_channels=256),
    # ConvolutionalLayer(
...
optimizer = SGD(model.parameters(), weight_decay=0.0001, lr=0.028, momentum=0.9)
...
# swa = StochasticWeightAveraging(learner, average_after=2345, update_every=670)
# callbacks = [
...
#     LRSchedulerCB(CosineAnnealingLR(optimizer, eta_min=0.007, T_max=335)),
#     swa,
#     LossLogger(),
