...
from nntoolbox.components import DropConnect, JitDropConnect
...
cell = DropConnect(RecurrentHighwayCell(128, 256, 1), weight_names=["weight_s"], ps=[0.5])
...
for _ in range(1000):
    optimizer.zero_grad()
    l = loss(cell(inp), target)
    l.backward()
    optimizer.step()
    print(l)

# layer = JitRNNLayer(RecurrentHighwayCell, 128, 256, 3)
# layer = JitLSTMLayer(MILSTMCell, 128, 256)
